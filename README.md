# Countries-SQL-Oracle
This is a SQL dump for a script to create a countries table in Oracle format.

This is a copy of data from [countries.sql](https://gist.github.com/adhipg/1600028) by Adhipg. I have included relevant updates from the comment section.

This is not a 100% accurate list, but it's the most accurate I've found for ease-of-use.

```
-- Run block between each code line seperately (except the insert block)
...
create table "COUNTRIES" 
    (	
        ...
        "CREATED_ON"        TIMESTAMP (9) WITH TIME ZONE NOT NULL ENABLE, 
        "CREATED_BY"        VARCHAR2(100) NOT NULL ENABLE, 
        "UPDATED_ON"        TIMESTAMP (9) WITH TIME ZONE NOT NULL ENABLE, 
        "UPDATED_BY"        VARCHAR2(100) NOT NULL ENABLE, 
    );

/

-- Alter table and create a primary key constraint.

alter table "COUNTRIES"
    add constraint "COUNTRIES_COUNTRIES_ID_PK" -- Note: My naming convention for PKs is TABLE_IDCOLUMN_PK
...
create sequence "SEQ_COUNTRIES" minvalue 1 maxvalue 99999999999 increment by 1 start with 1 nocache nocycle

/

-- Create trigger that will populate the primary key column and audit columns.

create or replace editionable trigger "COUNTRIES" 
  for insert 
  or update 
  ...
  begin
    if inserting then
      if :NEW.COUNTRIES_ID is null then 
        :NEW.COUNTRIES_ID := SEQ_COUNTRIES.nextval;
      end if;

      :NEW.CREATED_ON := CURRENT_TIMESTAMP;
      :NEW.CREATED_BY := coalesce(sys_context('APEX$SESSION', 'APP_USER'), 'NULL');
    end if;

    if inserting or updating then
      :NEW.UPDATED_ON := CURRENT_TIMESTAMP;
      :NEW.UPDATED_BY := coalesce(sys_context('APEX$SESSION', 'APP_USER'), 'NULL');
    end if;
...
alter trigger "TRG_COUNTRIES" enable;
...
insert into "COUNTRIES" 
    ...
    select 'AF', 'AFGHANISTAN', 'Afghanistan', 'AFG', 4, 93 from dual union all
    ...
    select 'VA', 'VATICAN CITY', 'Vatican City', 'VAT', 336, 39 from dual union all
    ...
    select 'RO', 'ROMANIA', 'Romania', 'ROU', 642, 40 from dual union all
    select 'RU', 'RUSSIAN FEDERATION', 'Russian Federation', 'RUS', 643, 7 from dual union all
    ...
    select 'ZW', 'ZIMBABWE', 'Zimbabwe', 'ZWE', 716, 263 from dual union all
    
-- Update 1

    select 'RS', 'SERBIA', 'Serbia', 'SRB', 688, 381 from dual union all
    select 'ME', 'MONTENEGRO', 'Montenegro', 'MNE', 499, 382 from dual union all
    select 'AX', 'ALAND ISLANDS', 'Aland Islands', 'ALA', 248, 358 from dual union all
    select 'BQ', 'BONAIRE, SINT EUSTATIUS AND SABA', 'Bonaire, Sint Eustatius and Saba', 'BES', 535, 599 from dual union all
    select 'CW', 'CURACAO', 'Curacao', 'CUW', 531, 599 from dual union all
    select 'GG', 'GUERNSEY', 'Guernsey', 'GGY', 831, 44 from dual union all
    select 'IM', 'ISLE OF MAN', 'Isle of Man', 'IMN', 833, 44 from dual union all
    select 'JE', 'JERSEY', 'Jersey', 'JEY', 832, 44 from dual union all
    select 'XK', 'KOSOVO', 'Kosovo', 'XKX', 0, 381 from dual union all
    select 'BL', 'SAINT BARTHELEMY', 'Saint Barthelemy', 'BLM', 652, 590 from dual union all
    select 'MF', 'SAINT MARTIN', 'Saint Martin', 'MAF', 663, 590 from dual union all
    select 'SX', 'SINT MAARTEN', 'Sint Maarten', 'SXM', 534, 1 from dual union all
    select 'SS', 'SOUTH SUDAN', 'South Sudan', 'SSD', 728, 211 from dual;
```
